...
namespace DXF.Importador_DXF
{
    public class DXFMatrix
    {
        ...
        public DXFVertex PtXMat(DXFVertex P)
        {
            DXFVertex Result = new DXFVertex();
            Result.X = Part(0, P);
            Result.Y = Part(1, P);
            Result.Z = Part(2, P);
            return Result;
        }
        private float Part(int I, DXFVertex P)
        {
            return (P.X * data[0, I] + P.Y * data[1, I] + P.Z * data[2, I] + data[3, I]);
        }
        public static DXFMatrix StdMat(DXFVertex S, DXFVertex P)
        {
            DXFMatrix Result = new DXFMatrix();
            Result.data[0, 0] = S.X;
            Result.data[1, 1] = S.Y;
        ...
        private static void MatOffset(DXFMatrix M, DXFVertex P)
        {
            M.data[3, 0] = P.X;
            M.data[3, 1] = P.Y;
            M.data[3, 2] = P.Z;
