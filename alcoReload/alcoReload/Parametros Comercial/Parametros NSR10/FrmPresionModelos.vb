...
Public Class FrmPresionDisenos
#Region "Variables"
    Private mPresionModelo As ClsPresionModelo
...
#End Region
    ...
    Private Sub FrmPresionDisenos_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            mPresionModelo = New ClsPresionModelo
            Dim objLista As List(Of presionModelos) = mPresionModelo.traerTodos(dwItems.TablaDatos)
            cargarEstados()
        Catch ex As Exception
        ManejoExcepciones.MensajeExcepcion.Show(ex, ManejoExcepciones.TipoExcepcion.Critica)
        End Try
    ...
    Private Function Validar() As Boolean
        Try
            ...
            If tform = ClsEnums.TiOperacion.INSERTAR Then
                If mPresionModelo.TraerxDescripcion(txtDescripcion.Text) = True Then
    ...
    Private Sub GuardadoTotal_Click(sender As System.Object, e As System.EventArgs)
        Try
            If Validar() Then
                ...
                If tform = ClsEnums.TiOperacion.INSERTAR Then
                    mPresionModelo.Ingresar(My.Settings.UsuarioActivo, txtDescripcion.Text, cmbEstado.SelectedValue, cbValorDefecto.Checked)
                ElseIf tform = ClsEnums.TiOperacion.MODIFICAR Then
                    mPresionModelo.modificar(txtDescripcion.Text, My.Settings.UsuarioActivo, cmbEstado.SelectedValue, curid, cbValorDefecto.Checked)
                End If
                MsgBox("La información se guardo exitosamente", MsgBoxStyle.Information)
                btnLimpiar_Click(Nothing, Nothing)
                FrmPresionDisenos_Load(Nothing, Nothing)
            Else
                Return
            End If
        Catch ex As Exception
