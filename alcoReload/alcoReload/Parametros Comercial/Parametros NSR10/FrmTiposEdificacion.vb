...
Public Class FrmTiposEdificacion
#Region "Variables"
    Private mTEdificacion As ClsTipoEdificacion
...
#Region "Procedimientos"
    ...
    Private Sub Modificar()
        Try
            ...
            curid = Convert.ToInt32(r.Cells(id_Tipo_Edificacion.DataPropertyName).Value)
            txtDescripcion.Text = r.Cells(Tipo_Edificacion.DataPropertyName).Value.ToString()
...
#End Region
    ...
    Private Sub Frm_Deactivate(sender As Object, e As System.EventArgs) Handles Me.Deactivate
        ...
        End Try
    End Sub
    Private Sub FrmTiposEdificacion_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            mTEdificacion = New ClsTipoEdificacion
            Dim objLista As List(Of tipoEdificacion) = mTEdificacion.traerTodos(dwItems.TablaDatos)
            cargarEstados()
        Catch ex As Exception
        ManejoExcepciones.MensajeExcepcion.Show(ex, ManejoExcepciones.TipoExcepcion.Critica)
        End Try
    ...
    Private Function Validacion() As Boolean
        Try
            ...
            If mTEdificacion.TraerxDescripcion(txtDescripcion.Text) = True Then Return False
            Return True
        Catch ex As Exception
            Return False
        ManejoExcepciones.MensajeExcepcion.Show(ex, ManejoExcepciones.TipoExcepcion.Critica)
    ...
    Private Sub GuardadoTotal_Click(sender As System.Object, e As System.EventArgs)
        Try
            ...
            If objValidacion.TextBoxDigitado(txtDescripcion, ErrorProvider1) Then
                Dim mutiimagen As New ClsUtilidadesImagenes
                If tform = ClsEnums.TiOperacion.INSERTAR Then
                    mTEdificacion.Ingresar(My.Settings.UsuarioActivo, txtDescripcion.Text,
                                           cmbEstado.SelectedValue, Convert.ToInt32(cbValorDefecto.Checked))
                ElseIf tform = ClsEnums.TiOperacion.MODIFICAR Then
                    mTEdificacion.Modificar(txtDescripcion.Text, My.Settings.UsuarioActivo,
                                            cmbEstado.SelectedValue, curid, Convert.ToInt32(cbValorDefecto.Checked))
                End If
                MsgBox("La información se guardo exitosamente", MsgBoxStyle.Information)
                btnLimpiar_Click(Nothing, Nothing)
                FrmTiposEdificacion_Load(Nothing, Nothing)
            Else
                Return
            End If
        Catch ex As Exception
