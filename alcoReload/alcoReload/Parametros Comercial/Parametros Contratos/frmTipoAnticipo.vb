...
#Region "Procedimientos de usuario"
    ...
    Private Sub GuardadoTotal_Click(sender As System.Object, e As System.EventArgs)
        Try
            If _tipoOperacom = ClsEnums.TiOperacion.INSERTAR Then
                If _validacion.TextBoxDigitado(txtDescripcion, ErrorProvider1) And _validacion.TextBoxDigitado(txtNombre, ErrorProvider1) Then
                    _objTipoAnticipo = New clsTipoAnticipo
                    _objTipoAnticipo.insert(Now, My.Settings.UsuarioActivo, txtNombre.Text, txtDescripcion.Text, My.Settings.UsuarioActivo, Now, cmbEstado.SelectedValue)
                End If
                MsgBox("La información se guardo exitosamente", MsgBoxStyle.Information)
                btnLimpiar_Click(Nothing, Nothing)
                frmTipoAnticipo_Load(Nothing, Nothing)
            Else
                If _validacion.TextBoxDigitado(txtDescripcion, ErrorProvider1) Then
                    _objTipoAnticipo = New clsTipoAnticipo
                    _objTipoAnticipo.Update(_idToEdit, txtNombre.Text, txtDescripcion.Text, My.Settings.UsuarioActivo, cmbEstado.SelectedValue)
                End If
    ...
    End Sub
#End Region
    Private Sub frmTipoAnticipo_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            _objTipoAnticipo = New clsTipoAnticipo
            dwItems.TablaDatos = Nothing
            Dim listaObjetosContratos As List(Of TipoAnticipos) = _objTipoAnticipo.selectAll(dwItems.TablaDatos)
            cargarEstados()
        Catch ex As Exception
            ManejoExcepciones.MensajeExcepcion.Show(ex, ManejoExcepciones.TipoExcepcion.Critica)
        End Try
    ...
    Private Sub frmTipoAnticipo_Activated(sender As Object, e As EventArgs) Handles Me.Activated
        Try
            ErrorProvider1.Clear()
    ...
    Private Sub dw_CellMouseClick(sender As Object, e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dwItems.CellMouseClick
        ...
        End Try
    End Sub
    Private Sub frmTipoAnticipo_Deactivate(sender As Object, e As EventArgs) Handles Me.Deactivate
        Try
            Dim btnsParcial As RibbonButton = DirectCast(Me.MdiParent, frmInicial).btnguardarParcial
            Dim btnsTotal As RibbonButton = DirectCast(Me.MdiParent, frmInicial).btnGuardarTotal
            Dim btnguardar As RibbonButton = DirectCast(Me.MdiParent, frmInicial).btnGuardar
