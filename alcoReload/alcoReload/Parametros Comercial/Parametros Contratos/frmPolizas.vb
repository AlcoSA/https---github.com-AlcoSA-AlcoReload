...
#Region "Variables"
    Private _objPolizas As clsPolizas
...
#Region "Procedimientos Usuarios"
        ...
        Try
            If _tipoOperacom = ClsEnums.TiOperacion.INSERTAR Then
                If _validacion.TextBoxDigitado(txtDescripcion, ErrorProvider1) Then
                    _objPolizas = New clsPolizas
                    _objPolizas.insert(My.Settings.UsuarioActivo, txtDescripcion.Text, nPorcentaje.Value,
                                       cmbEstado.SelectedValue)
                End If
                MsgBox("La información se guardo exitosamente", MsgBoxStyle.Information)
                btnLimpiar_Click(Nothing, Nothing)
                frmPolizas_Load(Nothing, Nothing)
            Else
                If _validacion.TextBoxDigitado(txtDescripcion, ErrorProvider1) Then
                    _objPolizas = New clsPolizas
                    _objPolizas.update(_idToEdit, txtDescripcion.Text, Convert.ToDecimal(nPorcentaje.Value),
    ...
    Private Sub Modificar()
        Try
            ...
            nPorcentaje.Value = r.Cells(PorcentajeDefecto.DataPropertyName).Value
...
#Region "Procedimientos Controles"
    Private Sub frmPoliza_Activated(sender As Object, e As EventArgs) Handles Me.Activated
        Try
            ErrorProvider1.Clear()
    ...
    Private Sub frmContrato_Deactivate(sender As Object, e As EventArgs) Handles Me.Deactivate
        Try
            Dim btnsParcial As RibbonButton = DirectCast(Me.MdiParent, frmInicial).btnguardarParcial
            Dim btnsTotal As RibbonButton = DirectCast(Me.MdiParent, frmInicial).btnGuardarTotal
            Dim btnguardar As RibbonButton = DirectCast(Me.MdiParent, frmInicial).btnGuardar
    ...
    Private Sub frmPolizas_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            _objPolizas = New clsPolizas
            Dim listapoliza As List(Of poliza) = _objPolizas.traerTodos(dwItems.TablaDatos)
            cargarEstados()
        Catch ex As Exception
            ManejoExcepciones.MensajeExcepcion.Show(ex, ManejoExcepciones.TipoExcepcion.Critica)
        End Try
