...
Public Class FrmOrigenesNotas
#Region "Varianles"
    Dim mOrigenNota As ClsOrigenNotas
...
#End Region
    ...
    Private Sub FrmOrigenesNotas_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            mOrigenNota = New ClsOrigenNotas
            Dim objLista As List(Of origen) = mOrigenNota.traerTodos(dwItems.TablaDatos)
            cargarEstados()
        Catch ex As Exception
            ManejoExcepciones.MensajeExcepcion.Show(ex, ManejoExcepciones.TipoExcepcion.Critica)
        End Try
        ...
        Try
            If Validar() Then
                If tform = ClsEnums.TiOperacion.INSERTAR Then
                    mOrigenNota.Insertar(My.Settings.UsuarioActivo, txtNombre.Text, txtDescripcion.Text,
                                         cmbEstado.SelectedValue)
                ElseIf tform = ClsEnums.TiOperacion.MODIFICAR Then
                    mOrigenNota.Modificar(curid, txtNombre.Text, txtDescripcion.Text, My.Settings.UsuarioActivo, cmbEstado.SelectedValue)
                End If
                MsgBox("La información se guardo exitosamente", MsgBoxStyle.Information)
                btnLimpiar_Click(Nothing, Nothing)
                FrmOrigenesNotas_Load(Nothing, Nothing)
            Else
                Return
            End If
        Catch ex As Exception
