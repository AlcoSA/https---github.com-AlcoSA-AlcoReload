...
Public Class frmTiposOrdenesTrabajo
#Region "Variables"
...
#Region "Prodecimientos"
    ...
    End Function
#End Region
    Private Sub frmTiposOrdenesTrabajo_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            cargarEstados()
            Dim mTiposOrdenesTrabajo = New ClsTiposOrdenesTrabajo
            mTiposOrdenesTrabajo.TraerTodos(dwItems.TablaDatos)
        Catch ex As Exception
            ManejoExcepciones.MensajeExcepcion.Show(ex, ManejoExcepciones.TipoExcepcion.Critica)
        End Try
    End Sub
    ...
    Private Sub GuardadoTotal_Click(sender As Object, e As EventArgs)
        Try
            If validacion() Then
                Dim mTiposOrdTrabajo As New ClsTiposOrdenesTrabajo
                If tform = ClsEnums.TiOperacion.INSERTAR Then
                    mTiposOrdTrabajo.Insertar(My.Settings.UsuarioActivo, txtDescripcion.Text, cmbEstado.SelectedValue)
                ElseIf tform = ClsEnums.TiOperacion.MODIFICAR Then
                    mTiposOrdTrabajo.Modificar(txtDescripcion.Text, My.Settings.UsuarioActivo, cmbEstado.SelectedValue, curId)
                End If
                MessageBox.Show("La información se ha guardado exitosamente", "",
                                MessageBoxButtons.OK, MessageBoxIcon.Information)
                btnLimpiar_Click(Nothing, Nothing)
                frmTiposOrdenesTrabajo_Load(Nothing, Nothing)
            End If
        Catch ex As Exception
            ManejoExcepciones.MensajeExcepcion.Show(ex, ManejoExcepciones.TipoExcepcion.Critica)
        End Try
