...
Public Class FrmTiposIdentificacion
#Region "Variables"
    Private mtiposidentificacion As ClsTiposIdentificacion
...
#End Region
    ...
    Private Sub Frm_Deactivate(sender As Object, e As System.EventArgs) Handles MyBase.Deactivate
        Try
            Dim btnsParcial As RibbonButton = DirectCast(Me.MdiParent, frmInicial).btnguardarParcial
            Dim btnsTotal As RibbonButton = DirectCast(Me.MdiParent, frmInicial).btnGuardarTotal
            Dim btnguardar As RibbonButton = DirectCast(Me.MdiParent, frmInicial).btnGuardar
        ...
        End Try
    End Sub
    Private Sub FrmTiposIdentificacion_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            mtiposidentificacion = New ClsTiposIdentificacion
            Dim objLista As List(Of TiposIdentificacion) = mtiposidentificacion.traerTodos(dwItems.TablaDatos)
            cargarEstados()
        Catch ex As Exception
        ManejoExcepciones.MensajeExcepcion.Show(ex, ManejoExcepciones.TipoExcepcion.Critica)
        End Try
    ...
    Private Function Validar() As Boolean
        Try
            ...
            If tform = ClsEnums.TiOperacion.INSERTAR Then
                If mtiposidentificacion.TraerxDescripcion(txtDescripcion.Text) = True Then
                    ErrorProvider1.SetError(txtDescripcion, "Ya existe este registro")
                    Return False
                End If
                ErrorProvider1.Clear()
                If mtiposidentificacion.TraerxPrefijo(txtPrefijo.Text) = True Then
                    ErrorProvider1.SetError(txtPrefijo, "Ya existe este registro")
                    Return False
                End If
                ErrorProvider1.Clear()
            End If
    ...
    Private Sub GuardadoTotal_Click(sender As System.Object, e As System.EventArgs)
        Try
            If Validar() Then
                ...
                If tform = ClsEnums.TiOperacion.INSERTAR Then
                    mtiposidentificacion.Ingresar(My.Settings.UsuarioActivo, txtPrefijo.Text, txtDescripcion.Text, cmbEstado.SelectedValue,
                                                  Convert.ToInt32(cbValorPorDefecto.Checked))
                ElseIf tform = ClsEnums.TiOperacion.MODIFICAR Then
                    mtiposidentificacion.Modificar(txtPrefijo.Text, txtDescripcion.Text, My.Settings.UsuarioActivo, cmbEstado.SelectedValue,
                                                   curid, Convert.ToInt32(cbValorPorDefecto.Checked))
                End If
                MsgBox("La información se guardo exitosamente", MsgBoxStyle.Information)
                btnLimpiar_Click(Nothing, Nothing)
                FrmTiposIdentificacion_Load(Nothing, Nothing)
            Else
                Return
            End If
        Catch ex As Exception
