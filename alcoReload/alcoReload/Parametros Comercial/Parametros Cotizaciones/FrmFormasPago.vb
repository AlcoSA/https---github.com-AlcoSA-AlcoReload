...
#Region "Procedimientos"
    ...
    Private Sub Modificar()
        Try
            ...
            curid = Convert.ToInt32(r.Cells(id_Forma_Pago.DataPropertyName).Value)
            txtDescripcion.Text = r.Cells(Forma_Pago.DataPropertyName).Value.ToString()
...
#End Region
    ...
    Private Sub FrmFormasPago_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            Dim objLista As List(Of formaPago) = mFormaPago.traerTodos(dwItems.TablaDatos)
            cargarEstados()
        Catch ex As Exception
            ManejoExcepciones.MensajeExcepcion.Show(ex, ManejoExcepciones.TipoExcepcion.Critica)
        End Try
    ...
    Private Function Validar()
        Try
            ...
            If tform = ClsEnums.TiOperacion.INSERTAR Then
                If mFormaPago.TraerxDescripcion(txtDescripcion.Text) = True Then
    ...
    Private Sub GuardadoTotal_Click(sender As System.Object, e As System.EventArgs)
        Try
            If Validar() Then
                If tform = ClsEnums.TiOperacion.INSERTAR Then
                    mFormaPago.Ingresar(My.Settings.UsuarioActivo, txtDescripcion.Text, cmbEstado.SelectedValue,
                                        Convert.ToInt32(cbValorPorDefecto.Checked))
                ElseIf tform = ClsEnums.TiOperacion.MODIFICAR Then
                    mFormaPago.Modificar(txtDescripcion.Text, My.Settings.UsuarioActivo, cmbEstado.SelectedValue, curid,
                                         Convert.ToInt32(cbValorPorDefecto.Checked))
                End If
                MsgBox("La información se guardo exitosamente", MsgBoxStyle.Information)
                btnLimpiar_Click(Nothing, Nothing)
                FrmFormasPago_Load(Nothing, Nothing)
            Else
                Return
            End If
        Catch ex As Exception
