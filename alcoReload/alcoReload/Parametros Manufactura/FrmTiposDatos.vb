...
Public Class FrmTiposDatos
#Region "Variables"
    Private mtipodato As ClsTiposDatos
...
#End Region
    ...
    Private Sub Frm_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Try
            mtipodato = New ClsTiposDatos
            Dim objList As List(Of TipoDato) = mtipodato.TraerTodos(dwItems.TablaDatos)
            cargarEstados()
        Catch ex As Exception
        ManejoExcepciones.MensajeExcepcion.Show(ex, ManejoExcepciones.TipoExcepcion.Critica)
        End Try
        ...
        Try
            If Validacion() Then
                If tform = ClsEnums.TiOperacion.INSERTAR Then
                    mtipodato.Ingresar(My.Settings.UsuarioActivo, txtNombre.Text, txtDescripcion.Text, cmbEstado.SelectedValue)
                ElseIf tform = ClsEnums.TiOperacion.MODIFICAR Then
                    mtipodato.Modificar(curid, My.Settings.UsuarioActivo, txtNombre.Text, txtDescripcion.Text, cmbEstado.SelectedValue)
                End If
            ...
            End If
        Catch ex As Exception
            If ex.Message.Contains("UNIQUE KEY 'UQ__ti020_ti__042822DE5C6CB6D7'") Then
                ErrorProvider1.Clear()
                ErrorProvider1.SetError(txtNombre, "Ya existe un tipo de dato con este nombre. Verifique la información")
                Return
            End If
        ManejoExcepciones.MensajeExcepcion.Show(ex, ManejoExcepciones.TipoExcepcion.Critica)
        End Try
